...
| behaviour.md §3.5: service-due jobs, and CloseRide waits for riders to leave | 0x8005BE44 selects the **upgrade queue**; state 54 calls the paid 0x8009C56C at 0x80096AA4. 0x8009C1E8 tests A+0xEC against `10*(w+h)<<12` (0x8009C260..288); 0x8009EEE4/EFA8 raise/lower that progress. It does not count riders. rides.md §0 item 3 already records this disagreement. | **Resolved 2026-09-20 (mechanic-fix):** Mechanic ports the READ. `IMechanicWorld.TryClaimQueuedUpgrade` / `CompleteUpgrade` replace the service names; `RideClosing` holds the A+0xEC arithmetic and state 17's separate `== 0` predicate. behaviour.md §3.5 carries the correction. |
| behaviour.md §3.5 / Mechanic: both coin-flip orders try both kinds of job | Service-first failure calls **0x8005BE44 again**, with repair flag 1, at 0x80096E4C..E64; repair-first uses 0x8005BCB0 then 0x8005BE44. | **Resolved 2026-09-20 (mechanic-fix):** `Mechanic.Idle` calls the queue twice in the upgrade-first order (`TryClaimQueuedUpgradeAsRepair` is the dead second call) and never the broken-ride selector there. |
