...
| `research.md` + `research-mutations.json` | researcher work/effort, five fixed-point topics, per-park catalogue and tier eligibility, ordered unlock choices, save precision, research-to-mechanic upgrade integration; source disagreements and restoring mutation sweep |
...
| `rider-positions.md` | where a rider is while a flat ride runs: no status hook writes it; the draw virtual pairs riders with the model's seat list (bone indices at the end of the mesh) and either draws the sprite in place (54 rides, nothing written) or writes x/z and forces anim 13 (5 bounce rides, `record+0xBC`); seat-slot table for all 59 flat rides, 8 with none |
| `people-sprites.md` | how a WALKING person's sprite is picked: five stored facings × eight frames from the FOLIO person resource (not 8×5), fold + X-mirror by `(facing + cameraYaw/45°) & 7`, the guest remap `0x800F1ECC`, the seven non-walk poses and which states show them, the body choice in slot 45, the per-drawable 4-draw frame counter, ride-cam half-size people; corrections to `PeopleSheet.cs` |
...
| `advisor-presentation.md` | where the advisor is DRAWN (own camera, FOLIO entry 0, scale/spin from +0xE8/+0xEC, gesture and face sub-meshes, 50 Hz frame clock), the caption as a message-list card (badge, tabs, 260-px box, draw-time word wrap, HUD font), the two payload fields (kind + object, no format codes), and the modal Triangle dismissal with the button tables decoded |
